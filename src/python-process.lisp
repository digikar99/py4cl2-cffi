...
(defun %pystart-standard (&key (verbose t) (release-gil t))
...
(defun %py-dedicated-thread-loop ()
  (destructuring-bind (fun &rest args)
    ...
    (push (handler-case (multiple-value-list (apply fun args))
            (error (c)
              (setf *pymain-error* c)
              c))
          *pymain-result-stack*)))
  ...
  (with-verbosity (ecase +python-call-mode+
    ...
    (thread-global-let ((*python-state* :initializing))
      (ecase +python-call-mode+
        (:dedicated-thread (%pystart-dedicated-thread :verbose verbose))
        (:standard (%pystart-standard :verbose verbose)))))
