...
(defun %pystart-dedicated-thread ()
  (setq *pymain-thread*
        (bt:make-thread
         (lambda ()
           ;; Wait for input
  ...
  (funcall/dedicated-thread #'%pystart-standard))
...
(defun pystart ()
  ...
  (thread-global-let ((*python-state* :initializing))
    (ecase +python-call-mode+
      (:dedicated-thread (%pystart-dedicated-thread))
      (:standard (%pystart-standard))))
